...
class ApiTest(unittest.TestCase):
   ...
   def test_base_api_url(self):
      self.assertEqual( app.url_for_endpoint('pippo') , '/api/v1.0/pippo/' )

   def test_base_api_url_with_params(self):
      self.assertEqual( app.url_for_endpoint('pippo/<float:age>') , '/api/v1.0/pippo/<float:age>/' )
   ...
   def test_buildings_by_id_with_valid_bid(self):
      response = self.app.get('/api/v1.0/buildings/aaaa/')
